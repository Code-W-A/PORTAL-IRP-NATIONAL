...
export default function SetariStructuraPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/assets/sigle");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      <div className="max-w-7xl mx-auto px-6 py-8">
              ...
              {logoUrl && (
                <div className="text-center">
                  <div className="text-xs font-medium text-gray-700 mb-2">
                    {decodeURIComponent(logoUrl.split("/").pop() || "")}
                  </div>
                  <div className="inline-block p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <img src={logoUrl} alt="logo" className="h-40 mx-auto rounded object-contain" />
                  </div>
        ...
        <div className="mt-8 bg-white rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-100/50 p-6">
          ...
          <div className="h-[85vh] bg-gray-50 rounded-xl overflow-hidden">
            {previewUrl ? (
              <iframe 
                title="pdf-preview" 
